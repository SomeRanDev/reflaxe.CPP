...
class Compiler_Includes extends SubCompiler {
	...
	function handleSpecialIncludeMeta(m: MetaAccess, header: Bool = true) {
		final anonUtil = m.extractParamsFromFirstMeta(":includeHaxeUtils");
		if(anonUtil != null) {
			IComp.addHaxeUtilHeader(anonUtil.length > 0 ? anonUtil[0] : header);
		}

		final anonUtil = m.extractParamsFromFirstMeta(":includeAnonUtils");
		if(anonUtil != null) {
			IComp.addAnonUtilHeader(anonUtil.length > 0 ? anonUtil[0] : header);
		}

		final typeUtil = m.extractParamsFromFirstMeta(":includeTypeUtils");
		if(typeUtil != null) {

...
haxe::Exception::Exception(std::string message, std::optional<haxe::Exception> previous, std::optional<std::any> native): _order_id(generate_order_id()) {
	std::exception();
...
haxe::Exception haxe::Exception::caught(std::any value) {
	...
	return haxe::Exception(tempString);
}

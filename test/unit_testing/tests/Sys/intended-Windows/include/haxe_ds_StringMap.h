...
public:
	...
	std::shared_ptr<KeyValueIterator<std::string, T>> keyValueIterator() {
		return std::make_shared<KeyValueIterator<std::string, T>>(std::make_shared<haxe::iterators::MapKeyValueIterator<std::string, T>>(weak_from_this().expired() ? std::make_shared<haxe::ds::StringMap<T>>(*this) : shared_from_this()));
	}
	...
	std::string toString() {
		...
		bool first = true;
		std::shared_ptr<haxe::IMap<std::string, T>> _g_map = weak_from_this().expired() ? std::make_shared<haxe::ds::StringMap<T>>(*this) : shared_from_this();
